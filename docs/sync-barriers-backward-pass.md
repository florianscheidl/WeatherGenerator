...
**Fix (implemented in `loss_module_physical.py` / `loss_module_ssl.py`; keeps
everything sync-free, without going back to synced scalars):** collect the
scalar loss terms in Python lists and reduce once per level with
`torch.stack(...).sum()` (one kernel instead of N chained adds). Because the
sum is flat, the per-stream loss collects into a single list across timesteps
and correspondences — the nesting only matters for the counters, which reduce
to one batched `(stack > 0).any()` per timestep and one `stack(flags).sum()`
per stream. No `torch.zeros(())` accumulators remain outside degenerate
fallbacks (a stacked sum needs no zero-init), and the per-channel "avg" logging
reduces with one stacked `nan_to_num().sum()` per (stream, loss function)
instead of one kernel per entry. Values *and* gradients verified equal to the
chained-accumulator version over randomized nesting structures (incl. spoofed
targets, zero losses, empty levels). Note: float summation order changes
(stacked sum vs sequential adds), so results are equivalent, not bit-identical.
