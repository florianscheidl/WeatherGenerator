...
| ✅ `e02d143e` + working tree | `assert ... <= max_tokens` on a GPU reduction, every forward | Checked on the host copy of the lens (`batch.tokens_lens_cpu`) before the `pe_embed` indexing (`engines.py:105-119`): `int(tokens_lens_cpu.sum(2).max())` is a pure CPU op, so the check is exact, runs every step, and never syncs. (An earlier try/except around the indexing didn't work: on CUDA an out-of-bounds gather is an async device-side assert, not an `IndexError`.) |
