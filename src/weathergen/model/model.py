...
from weathergen.model.utils import get_num_parameters, maybe_checkpoint
...
class Model(torch.nn.Module):
    ...
    def reset_parameters(self):
        def _reset_params(module):
            if isinstance(module, nn.Linear | LayerNorm | RMSNorm):
    ...
    def predict_decoders(
        ...
        tokens_nbors_lens = torch.full(
            (s[0] * s[1] + 1,), fill_value=9, dtype=torch.int64, device=tokens_nbors.device
        )
        ...
        for stream_name in self.stream_names:
            ...
            tc_tokens = maybe_checkpoint(tc_embed, t_coords, use_reentrant=False)
