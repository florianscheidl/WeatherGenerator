...
class StreamEmbedTransformer(torch.nn.Module):
    ...
    def forward_channels(self, x_in):
        # Direct call - positional_encoding_harmonic uses torch.no_grad() internally
        # so the large PE tensor is freed immediately after addition
        x = positional_encoding_harmonic(self.embed(x_in.transpose(-2, -1)))
